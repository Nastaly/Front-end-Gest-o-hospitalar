...
export function Dashboard({ user, onLogout }: DashboardProps) {
  ...
  const allMenuItems = [
    { id: 'consultas', label: 'Consultas', icon: Calendar, roles: ['admin', 'medico', 'recepcao'] },
    { id: 'triagem', label: 'Triagem', icon: ClipboardList, roles: ['admin', 'enfermeiro'] },
    { id: 'triagem-view', label: 'Visualizar Triagem', icon: ClipboardList, roles: ['medico'] },
    { id: 'cadastro-pacientes', label: 'Cadastro de Pacientes', icon: UserPlus, roles: ['admin', 'recepcao'] },
    { id: 'pacientes-view', label: 'Visualizar Pacientes', icon: UserPlus, roles: ['admin', 'recepcao'] },
    { id: 'registro-pacientes', label: 'Registro de Pacientes', icon: FileText, roles: ['admin', 'enfermeiro', 'recepcao'] },
    { id: 'prontuarios', label: 'Prontuários Médicos', icon: FileText, roles: ['admin', 'medico'] },
    { id: 'estoque', label: 'Estoque de Medicamentos', icon: Package, roles: ['admin', 'enfermeiro'] },
    { id: 'cadastro-funcionarios', label: 'Cadastro de Funcionários', icon: Users, roles: ['admin'], adminOnly: true },
    { id: 'cadastro-especialidades', label: 'Especialidades', icon: FileText, roles: ['admin'], adminOnly: true },
    { id: 'auditoria', label: 'Auditoria', icon: Shield, roles: ['admin'], adminOnly: true },
  ];
    ...
    if (!currentItem || !currentItem.roles.includes(user.role)) {
      return (
        <div className="flex flex-col items-center justify-center h-full">
          ...
          <p className="text-gray-500 mt-2">Entre em contato com o administrador do sistema.</p>
        </div>
      );
    }
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      >
        <div className="p-6 border-b border-blue-800">
          ...
          <span className={`inline-block px-3 py-1 rounded-full text-sm ${roleBadge.color}`}>
            {roleBadge.label}
          </span>
        </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <header className="bg-white shadow-sm p-4 flex items-center justify-between">
          ...
          <h1 className="text-gray-800">
            {menuItems.find(item => item.id === activeTab)?.label || 'Acesso Restrito'}
          </h1>
